<template>
	<view class="mian">
		...
		<view class="headPortrait" @click="pas_click">
			<view class="headPortrait_left">
				<view class="expression" v-if="userinfo.user">
					<view class="iconfont" style="font-size: 80upx;color: #FFFFFF;"
						v-if="userinfo.passengerVo.headImg == null || userinfo.passengerVo.headImg == ''">
						&#xe63c;
					</view>
		...
		<view class="tail">
			<view v-for="(item,index) in tabBar_tail" :key="index" class="tabbar_text" @click="navTos(item)"
				v-if="rotes(item.is)">
				<view class="tabbar_left">
					<view class="iconfont" style="color: #007aff;margin: 0 5%;font-size: 45upx;">{{item.imgNormal}}
					</view>
		...
		methods: {
			...
			navTos(item) {
				if (item.url == 'Apply_for') {
					...
					uni.navigateTo({
						url: '/pages-mydi/pages/mydi/IntentList/Intentlist'
					})
				}
			},
			pas_click() {
